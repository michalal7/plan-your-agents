...
The paper (v2.1.88) builds a value-tension argument on an **initialization-order gap**: hooks, MCP connections and settings resolution executing *before* the interactive trust dialog, with CVEs attributed to it. `40-config-safety.md` carries the same finding from Anthropic's own containment write-up — **and records that it was fixed by deferring hook execution.** So the specific vulnerability is closed; what survives is the general lesson already stated there: anything running at session start runs before the human has agreed to anything. Do not cite the paper's CVE framing as current.

**General caution for this source.** It analyses v2.1.88 against a KB tracking ~2.1.214 — roughly 126 releases. Every concrete surface in it (flag names, event counts, permission-mode lists, tool counts, file paths, numeric limits) is presumed stale and was deliberately not ingested. It is used **only** for design rationale the official docs do not explain, marked `(third-party source analysis, v2.1.88)`.
