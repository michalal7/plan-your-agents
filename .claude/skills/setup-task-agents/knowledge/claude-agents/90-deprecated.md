...
- ~~**`SLASH_COMMAND_TOOL_CHAR_BUDGET` — not documented.**~~ **WITHDRAWN, same day.** It *is* on `/en/env-vars`: it overrides the character budget for skill metadata shown to the Skill tool, scaling at 1% of the context window with an 8,000-character fallback (legacy name kept for compatibility). The entry existed only because two separate fetches of that page came back **truncated** and absence-in-a-truncated-fetch was written down as absence-in-the-docs. `_state.json` has warned since run 3 that `/en/settings` renders truncated and does not list every key it owns — the same trap, one page over. **A truncated fetch can never establish a negative.** Kept visible rather than deleted, because this KB's whole value is that it does not assert what it has not checked.
...
## Superseded by newer evidence: the arXiv paper's pre-trust window (run 6, 2026-07-18)
The paper (v2.1.88) builds a value-tension argument on an **initialization-order gap**: hooks, MCP connections and settings resolution executing *before* the interactive trust dialog, with CVEs attributed to it. Anthropic's own containment write-up confirms the gap **for hooks and project-local settings, and states the fix: defer parsing and execution of project-local configuration until after the trust prompt** (`40-config-safety.md`). That much is closed. The **MCP-connection** part of the paper's framing is *not* covered by that write-up and was not verified either way here — do not repeat it as either live or fixed. What survives regardless is the general lesson: anything running at session start runs before the human has agreed to anything.

**General caution for this source.** It analyses v2.1.88 against a KB tracking ~2.1.214 — roughly 126 releases. Every concrete surface in it (flag names, event counts, permission-mode lists, tool counts, file paths, numeric limits) is presumed stale and was deliberately not ingested.

⚠️ **And the filter was not sufficient.** Screening out *surface* detail let a **mechanism** claim through as current fact: the paper's account of extension-mechanism context costs put MCP servers at the expensive end ("schemas always loaded"). That is wrong today — tool search defers MCP schemas by default, and the genuinely always-resident feature is CLAUDE.md. `PLAYBOOK` §1b was rewritten from `/en/features-overview`, which explains the whole topic better than the paper does. Two lessons: a mechanism claim from a version-drifted source needs doc verification exactly like a flag name does, and "the docs never explain this" is itself a claim to check before it justifies leaning on an unauthoritative source.
